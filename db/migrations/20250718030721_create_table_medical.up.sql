...
CREATE TABLE IF NOT EXISTS patient_yearly_benefit_claims (
                                                             id INT PRIMARY KEY AUTO_INCREMENT,
                                                             patient_id INT NOT NULL,
                                                             yearly_benefit_claim_id INT NOT NULL,
                                                             yearly_claim_remaining DECIMAL(18,2) NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT uq_pybc UNIQUE (patient_id, yearly_benefit_claim_id),
    CONSTRAINT fk_pybc_patient
    FOREIGN KEY (patient_id) REFERENCES patients(id)
                                                        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_pybc_ybc
    FOREIGN KEY (yearly_benefit_claim_id) REFERENCES yearly_benefit_claims(id)
                                                        ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB;
